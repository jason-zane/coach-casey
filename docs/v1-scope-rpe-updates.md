...
> - **RPE patterns across the week feed the interpretation.** RPE-vs-pace divergence trends are a primary input; *"You ran easy four times, RPE averaged 5 against your usual band of 3-4: worth watching"* is exactly the kind of read single-activity debriefs can't make.
...
## 6. Update §5 "Out of V1": Product exclusions (deferred, not violating thesis)
...
> - **RPE re-prompt copy and post-pause threshold tuning.** After a 5-skip pause, what does the re-prompt say? Does the post-pause skip threshold differ from the initial threshold? Currently 21-day pause and 5-skip threshold both used post-pause as placeholders for now.
