...
**1. Strava API developer application (V1-blocking — see §8).** Nothing Strava-dependent starts until this is approved.

**2. Infrastructure foundation (parallel with Strava application).**
- Supabase schema + RLS
...
**3. Strava integration (after application approval).**
- OAuth flow
...
- **Strava developer application approval.** Covered in `strava-api-compliance-note.md`. Nothing Strava-dependent can be built against production OAuth until approved. Application language must use Strava's own framing — "coaching platforms focused on providing feedback to users." Optionally email Strava developer relations for written confirmation first. Everything non-Strava can be built in parallel.
